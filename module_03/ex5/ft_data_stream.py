...
def generators():
    i = 1
    for event in data:
        yield (f"Event {i}: Player {event['player']} "
...
print(f"Login events: {count_login}")
...
gen_fib = ""
for x in generator_fibonacci(10):
    gen_fib += f"{x}, "
...
gen_prim = ""
for p in generator_prime(5):
    gen_prim += f"{p}, "
print(f"Fibonacci sequence (first 10): {gen_fib[:-2]}")
print(f"Prime numbers (first 5): {gen_prim[:-2]}")
